...
[blank, namevec] = xlsread(xlsname,range); %xlsread(filename,sheet,xlrange) reads excel files.  Note that "sheet" refers to the sheet name, not sheet number.
...
[blank, descmat] = xlsread(xlsname,range);
         descmat = descmat' ; 
end;
...
if ncodes >0;
    ...
    tcodemat = xlsread(xlsname,range);
...
dates = readtable(xlsname,'ReadVariableNames',0,'Range',range);
matlabdates = erase(dates{:,:},'M');
matlabdates = datenum(matlabdates, 'yyyymm'); %convert excel dates to matlab (since matlab starts in 1/1/2000 while excel starts in 1/1/1900)
datestring  = datestr(matlabdates,23); %mm/dd/yyyy form

%Get Data %
range   = strcat('B',num2str(firstrow_data),':',endcol,num2str(endrow_data));
datamat = xlsread(xlsname,range);
